...
export class AStar extends MinIndexedPQ {
  constructor(matrix, noise, coords, xOffset, yOffset) {
    super()
  ...
  async * search() {
    while (!this.isEmpty()) {
      ...
      const minNode = this.poll()

      const row = Math.floor(minNode / this.width)
      const col = minNode % this.width
      ...
      for (const neighbor of this.matrix[row][col]) {
        ...
        // Calculate the cost of exploring neighbor
        const costNeighbor = this.noise[nRow + this.yOffset][nCol + this.xOffset]

        // Compute a tentative travel cost
        const newCost = this.travelCost[row][col] + costNeighbor

        // If the tentative cost is cheaper than existing cost
        if (newCost < this.travelCost[nRow][nCol]) {
          this.travelCost[nRow][nCol] = newCost

          // Remember path via backwards map (neighbor -> current)
          this.map.set(nRow * this.width + nCol, minNode)
          ...
          if (!this.contains(nRow * this.width + nCol)) {
            this.insert(nRow * this.width + nCol, newCost + manhattan)

          } else {
            this.decreaseKey(nRow * this.width + nCol, newCost + manhattan)
          }
